...
$query1 = "SELECT * FROM `qp_table` WHERE (SetClaim='$varSetDateClaim' AND sessionType='$varSession')";
...
//
$query = "UPDATE `qp_table` SET `Status`='$varStatus',`Request_Approved_date`='$varRequest_Approved_date',`SetClaim`='$varSetDateClaim',`sessionType`='$varSession' WHERE `refID`='$varRefID'";
...
$subjectName = 'Quarantine Pass Schedule';
$yourName = $varFname;
$refID = $varRefID;
...
Hi! '.$yourName.'<br/>
                 <br/>
Your request for Quarantine Pass form has been accepted. 
...
//end of email sender-------------------------------
	header("location: Admin-QP.php?Approve=Succes");			
	...
	exit();
			...
			header("location: Admin-QP.php?Approve=Error");
	}
