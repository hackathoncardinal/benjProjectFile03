...
$txtSearch = $_POST['txtRequest_search'];
	...
	$queryRequest = mysqli_query($dbc,"SELECT * From qp_table where(status='$r1')");
	$queryUnclaimed = mysqli_query($dbc,"SELECT * From qp_table where(status='$r2')");
	$queryClaimed = mysqli_query($dbc,"SELECT * From qp_table where(status='$r3')");
		echo "<script>alert('Textbox Or Filter Column was Empty');</script>";
	}else{
	$queryRequest = mysqli_query($dbc,"SELECT * FROM `qp_table` WHERE Status='$r1' AND $col1 LIKE '%$txtSearch%'");
	//mysqli_num_rows($queryRequest)
	if(mysqli_num_rows($queryRequest) > 0){
		//echo "<script>alert('data Found');</script>";
	}else{ $queryRequest = mysqli_query($dbc,"SELECT * From qp_table where(status='$r1')"); 
...
$txtSearch = $_POST['txtRequest_search2'];
	...
	$queryUnclaimed = mysqli_query($dbc,"SELECT * FROM `qp_table` WHERE Status='$r2' AND $col1 LIKE '%$txtSearch%'");
	//mysqli_num_rows($queryRequest)
	if(mysqli_num_rows($queryUnclaimed) > 0){
		//echo "<script>alert('data Found');</script>";
	}else{ $queryUnclaimed = mysqli_query($dbc,"SELECT * From qp_table where(status='$r2')");
		 	echo "<script>alert('data not Found');</script>";
		 }
...
<head>
  <!-- Basic -->
    ...
    <meta name="viewport" content="width=device-width, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no">
     ...
     <!-- Site Metas -->
    <title>Admin Quaratine DB</title>  
                ...
                <div id="navbar" class="navbar-collapse collapse">
                    <ul class="nav navbar-nav navbar-right">
                       <li><a  href="Admin-Dashboard.php">Dashboard</a></li>
                        <li><a class="active" href="Admin-QP.php">Quarantine Pass DB</a></li>
...
<div class="container">
	<div style="text-align: -webkit-center;">
		<form method="POST" action="Admin-QP.php">
...
//                alert(msg);
           ...
           Passvalue1(therow.cells[0].innerHTML,therow.cells[1].innerHTML,therow.cells[2].innerHTML,therow.cells[3].innerHTML,therow.cells[4].innerHTML,therow.cells[5].innerHTML,therow.cells[6].innerHTML,therow.cells[7].innerHTML,therow.cells[8].innerHTML,therow.cells[9].innerHTML,therow.cells[10].innerHTML,therow.cells[11].innerHTML,therow.cells[12].innerHTML,therow.cells[13].innerHTML);
            window.location = "Dataseleted.php";
...
//                alert(msg);
           ...
           Passvalue1(therow.cells[0].innerHTML,therow.cells[1].innerHTML,therow.cells[2].innerHTML,therow.cells[3].innerHTML,therow.cells[4].innerHTML,therow.cells[5].innerHTML,therow.cells[6].innerHTML,therow.cells[7].innerHTML,therow.cells[8].innerHTML,therow.cells[9].innerHTML,therow.cells[10].innerHTML,therow.cells[11].innerHTML,therow.cells[12].innerHTML,therow.cells[13].innerHTML);
            window.location = "Admin-QP-Dataseleted-Unclaimed.php";
